...
def login(contents):
	return '<h1>Welcome. Enjoy your premium <a href="content.html">membership.</a></h1>'
def home(contents):
	return '<a href="../index.html">Home</a><br>Hello '+contents['username']+'.<br><a onclick="posts(0)">Posts</a> <a onclick="files()">Files</a> <a onclick="logout()">Logout</a> <a onclick="messages"> Messages</a> <a onclick="seeBash()">Bash</a> <a onlick="preferences()">Preferences</a><br><div id="actualcontent"></div>'

def posts(contents):
	postnames = os.listdir(folder()+'posts');
	postnames.sort(reverse=True)
	start = contents['data']['start']
	length = preferences(contents['username'])['postnumber']
